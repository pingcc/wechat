...
public class StudentServer implements IStudentServer {
    ...
    @Override
    public Student_HI getIdToStudentParams(JSONObject queryParamJSON) {
        long id = queryParamJSON.getLong("id");
        List<Student_HI> list= studentDao_hi.selectList(null);
        return studentDao_hi.selectById(id);

    }

    @Override
    public  Student_HI getIdToStudentBody(Student_HI studentHI) {
        HashMap map = new HashMap<String,String>();
        map.put("name",studentHI.getName());
        List<Student_HI> list = studentDao_hi.selectByMap(map);
        return list.get(0);
