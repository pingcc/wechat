...
public class IdCardVerification {
	 ...
	 * @return 错误信息或成功信息
	 */
	public static JSONObject IDCardValidate(String IDStr) {
		String Ai = "";
		// 判断号码的长度 15位或18位
		if (IDStr.length() != 15 && IDStr.length() != 18) {
			return new JSONObject().fluentPut("status","E").fluentPut("msg",LACKDIGITS);
		}
		...
		if (isNumeric(Ai) == false) {
			return new JSONObject().fluentPut("status","E").fluentPut("msg",LASTOFNUMBER);
		}
		...
		if (isDate(strYear + "-" + strMonth + "-" + strDay) == false) {
			return new JSONObject().fluentPut("status","E").fluentPut("msg",INVALIDBIRTH);
		}
		...
		try {
			if ((gc.get(Calendar.YEAR) - Integer.parseInt(strYear)) > 150
				...
				return new JSONObject().fluentPut("status","E").fluentPut("msg",INVALIDSCOPE);
			}
		...
		if (Integer.parseInt(strMonth) > 12 || Integer.parseInt(strMonth) == 0) {
			return new JSONObject().fluentPut("status","E").fluentPut("msg",INVALIDMONTH);
		}
		if (Integer.parseInt(strDay) > 31 || Integer.parseInt(strDay) == 0) {
			return new JSONObject().fluentPut("status","E").fluentPut("msg",INVALIDDAY);
		}
		...
		if (areacode.get(Ai.substring(0, 2)) == null) {
			return new JSONObject().fluentPut("status","E").fluentPut("msg",CODINGERROR);
		}

		if (isVarifyCode(Ai, IDStr) == false) {
			return new JSONObject().fluentPut("status","E").fluentPut("msg",INVALIDCALIBRATION);
		}

		return new JSONObject().fluentPut("status","S").fluentPut("msg",VALIDITY);
	}
	...
	public static void main(String[] args) {
		System.err.println(IDCardValidate("11010119900507549X"));
	}
