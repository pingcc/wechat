...
public class CalendarUtil {
	...
	public static String solarToLunar(String solarDate) throws Exception{
		...
		SimpleDateFormat formatter = new SimpleDateFormat("yyyyMMdd");
		Date myDate = formatter.parse(solarDate);
	...
	public static int getMonths(Date start,Date end){
		...
		if (startDay>endDay){ //1月17  大于 2月28
			...
			calendar.set(curYear, 2 - 1, 1);
			int daysOfMonth = calendar.getActualMaximum(Calendar.DAY_OF_MONTH);

			if (endDay==daysOfMonth){   //也满足一月
	...
	/**
	 * 计算当前年的阳历生日
	 * @param type 生日类型，1：阳历，2：阴历
	 * @param birthday 生日
	 * @author ZhangJun
	 * @createTime 2018/8/23
	 * @description 计算当前年的阳历生日
	 */
	public static Date getCurrentYearBirthday(String type,Date birthday) throws Exception{

		if(StringUtils.isBlank(type) || birthday == null){
			return null;
		}

		//返回当前年的生日（阳历或阴历）
		String year = SaafDateUtils.convertDateToString(SaafDateUtils.getDate(0),"yyyy");
		String month = SaafDateUtils.convertDateToString(birthday,"MM");
		String day = SaafDateUtils.convertDateToString(birthday,"dd");
		String birthday_str = year + "-" + month+"-"+day;

		//计算阳历生日
		if(org.apache.commons.lang.StringUtils.equalsIgnoreCase(type,"1")){
			//如果是阳历生日，返回
			return SaafDateUtils.string2UtilDate(birthday_str,"yyyy-MM-dd");
		}else{
			//如果是阴历，则计算阳历生日
			//闰月
			int LeapMonth = CalendarUtil.getLeapMonth(Integer.valueOf(year));

			String solarDate = CalendarUtil.lunarToSolar(year+month+day, Integer.valueOf(month)==LeapMonth);

			return SaafDateUtils.string2UtilDate(solarDate,"yyyyMMdd");
		}
	}

	public static void main(String[] args) throws  Exception{
		String str = "1988-10-16";
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		Date d1 = sdf.parse(str);
		Date ss = getCurrentYearBirthday("2",d1);
